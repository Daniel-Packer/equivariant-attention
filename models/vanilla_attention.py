...
def train(rng: jax.Array, X_train: jnp.ndarray, encoding: Callable[[jnp.ndarray], jnp.ndarray], n_keys: int, n_epochs: int, lr: float, f: Callable[[jnp.ndarray], jnp.ndarray], verbose: bool = False):
...
def plot_predictions(encoding, keys_hist, values_hist, cmap, ax, X_test = None):
    ...
    for i in range(0, n_epochs, int(n_epochs / 50)):
        ...
        X_test = jnp.linspace(-1.5, 1.5, num = 1_000)[:, None] if X_test is None else X_test
